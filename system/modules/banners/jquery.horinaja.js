...
jQuery.fn.extend({
	everyTime: function(interval, label, fn, times, belay) {
		return this.each(function() { jQuery.timer.add(this, interval, label, fn, times, belay); });
	},
	oneTime: function(interval, label, fn) {
		return this.each(function() { jQuery.timer.add(this, interval, label, fn, 1); });
	},
...
$.fn.Horinaja = function(settings) {
    ...
    return this.each(function() {
		...
		var po = 0;
		var id = 0;
		function fadeP(it, opac) {
            if (pagination) { $('#' + capture + ' > ol.horinaja_pagination > li:eq(' + it + ')').fadeTo("fast", opac); }
        }
		function moveP() {
			if (po != -((px * nCell) - px)) {
                $('#' + capture + ' > ul').animate({ left: (po-px) + "px" }, delai);
				po = po - px;
				fadeP(id, 0.4);
				id=id + 1;
				fadeP(id, 0.7);
			} else {
				$('#' + capture + ' > ul').animate({ left: "0px" }, delai);
				po = 0;
				fadeP(id, 0.4);
				id = 0;
				fadeP(id, 0.7);
	...
	if (pagination) {
		...
		for (i=1; i!=(nCell + 1); i++) {
            $('#' + capture + ' > ol.horinaja_pagination').append('<li><a style="width:' + wb + 'px;">' + i + '</a></li>');
        }
		...
		$('#' + capture + ' > ol.horinaja_pagination > li').each(function(i) {
		  $(this).bind('click', {index:i}, function(e) {
			...
			if (id > occ) {
				...
				po = po + (px * diff);
				id = occ;
				$('#' + capture + ' > ul').animate({ left: (po) + "px" }, delai);
			} else if (id < occ) {
				diff = occ - id;
				po = po - (px * diff);
				id = occ;
