...
<fieldset>
    <form name="sections" method="post" action="" class="form">
        <div id="drag">
            <table class="sortable">
                ...
                <tr class="{sections[class]}">
                    <td class="rowhandler"><div id="{sections[id]}" class="drag row"><img src="{ICONS}move.png" width="16" height="16" alt="[__Move]" /></div></td>
                    <td class="id"><input type="hidden" name="ids[]" value="{sections[id]}" />{sections[id]}</td>
                    <td class="title"><input type="hidden" name="titles[]" value="{sections[title]}" />{sections[title]}</td>
                    <td class="desc"><input type="hidden" name="descs[]" value="{sections[desc]}" />{sections[desc]}</td>
                    <td class="access center"><input type="hidden" name="accesses[]" value="{sections[access]}" />{sections[access]}</td>
        ...
        <p align="center">
            ...
            [if=sections]<input type="submit" name="action" value="[__Save]" class="submit" />[/if]
        </p>
    </form>
</fieldset>
