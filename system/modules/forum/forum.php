    ...
    if (!empty($section)) {
        ...
        if (!empty($topic) && !empty($category) && !empty($section)) {
                            ...
                            case 'edit':
                                ...
                                if (!empty($content[$topic]['opened'])) {
                                    if (empty($reply)) {
                                        ...
                                        $TEMPLATE->set('topic', $ed['id']);
                                        $TEMPLATE->set('title',  empty($REQUEST['title'])  ? $ed['title']  : $REQUEST['title']);
                                        $TEMPLATE->set('text',   empty($REQUEST['text'])   ? $ed['text']   : $REQUEST['text']);
                                        $TEMPLATE->set('opened', empty($REQUEST['opened']) ? $ed['opened'] : $REQUEST['opened']);
                                        $TEMPLATE->set('pinned', empty($REQUEST['pinned']) ? $ed['pinned'] : $REQUEST['pinned']);
                                        $TEMPLATE->set('moderator', USER::moderator('forum', $topic));
                                        $TEMPLATE->set('bbCodes', CMS::call('PARSER')->showBbcodesPanel('topic.text'));
                                        ...
                                        if (!empty($replies[$reply])) {
                                            ...
                                            if (USER::moderator('forum', $replies[$reply])) {
                                                ...
                                                $TEMPLATE->set('comment', $reply);
                                                $TEMPLATE->set('text', empty($REQUEST['text']) ? $replies[$reply]['text'] : $REQUEST['text']);
                                                $TEMPLATE->set('moderator', USER::moderator('forum') ? TRUE : NULL);
                                                $TEMPLATE->set('bbcodes', CMS::call('PARSER')->showBbcodesPanel('edit.text', !empty($output['moderator'])));
                                                SYSTEM::defineWindow('Edit', $TEMPLATE->parse());
                                            }
                                        }
                                    }
            ...
            if (!empty($REQUEST['new'])) {
                ...
                $TEMPLATE = new TEMPLATE(__DIR__.DS.'post.tpl');
                $TEMPLATE->set('new', TRUE);
                $TEMPLATE->set('title',     FILTER::get('REQUEST', 'title'));
                $TEMPLATE->set('text',      FILTER::get('REQUEST', 'text'));
                $TEMPLATE->set('moderator', FILTER::get('REQUEST', 'moderator'));
                $TEMPLATE->set('bbCodes', CMS::call('PARSER')->showBbcodesPanel('topic.text'));
                SYSTEM::defineWindow('New topic', $TEMPLATE->parse());
...
//                    $output[$ids[$i]]['last_link'] = $output[$ids[$i]]['link'].COMMENT.$content[$ids[$i]]['comments'];
                ...
                $TEMPLATE->set('post_allowed', USER::$logged_in ? TRUE : NULL);
                $TEMPLATE->set('topics', $output);
                SYSTEM::defineWindow($category['title'], $TEMPLATE->parse());
            ...
            $TEMPLATE->set($output);
            SYSTEM::defineWindow('Forum', $TEMPLATE->parse());
        }
    } else {
        #
        ...
        $TEMPLATE = new TEMPLATE(__DIR__.DS.'forum.tpl');
        $TEMPLATE->set($output);
        SYSTEM::defineWindow('Forum', $TEMPLATE->parse());
    }
